...
function Arkivum(config){
	...
	this.set_options = function(config){
                ...
                self.init_oc_link_options();
        };

    //an onclick for links to owncloud
    this.init_oc_link_options = function(){
        self.oc_link_click = 'onclick="window.open(this.href,\'targetWindow\',\''+
        'toolbar=no,'+
        'location=no,'+
        'status=no,'+
        'menubar=no,'+
...
//	this.get_files_data = function(in_loop=false){
	this.get_files_data = function(in_loop){
        var d = new Date();
		var url="/cgi/export/eprint/"+eprintid+"/ArkivumFiles/"+repoid+"-eprint-"+d.getTime()+".js";
		//console.log("url", url);
...
//		    if(in_loop && JSON.stringify(data.children) !== JSON.stringify(self.files_data.children)){
		//This will only trigger if a file has been added or removed
			...
			var files_then = 0;
			var files_now = self.count_key_instances(data,"children",0);

			if(self.files_data != undefined){
				var files_then = self.count_key_instances(self.files_data,"children",0);
			}
			if(in_loop && self.files_data != undefined && files_then != files_now ){
			...
			//repeat call every 10 seconds !? #TODO make this configurable
	...
	//	    self.render_shares();
		    console.log( "success", data );
		    j("#public_share_link").html('Download data: <a href="'+file_share_url+'/apps/files/?dir='+j("data file_target",data).text()+'" '+self.oc_link_click +'>'+j("data file_target",data).text()+'</a>');
	...
	this.render_shares = function(){
		...
		j(self.shares).each(function(i){
			if(j("share_type", this).text() == "3") //public
				j("ul.shares_list").append('<li>Public share link: <a href="'+file_share_url+'/s/'+j("token", this).text()+'" '+self.oc_link_click +'>'+j("token", this).text()+'</a></li>');
			if(j("share_type", this).text() == "0") //user
				j("ul.shares_list").append('<li>User share with <a href="'+file_share_url+'/apps/files/?dir=/'+eprintid+'" '+self.oc_link_click +'>'+j("share_with_displayname", this).text()+'</a></li>');
...
//			data.password = j("input[name='share_pw']").val();
	...
	this.get_user = function(){
		...
		j.ajax( url, {data: {username: username, action: 'get'}, dataType: "xml" } )
		  .done(function(data) {
			...
			j("input[name='oc_user']").val(username); //we won't really use this we get current_user from server
			j("input[name='oc_user']").prop('disabled', true);
	...
	this.render_user = function(){
		j("p.oc_user").html( );
		j(self.shares).each(function(i){
			j("ul.shares_list").append('<li><a href="'+file_share_url+'/'+j("token", this).text()+'" '+self.oc_link_click +'>'+j("token", this).text()+'</a></li>');
		});
	};
...
*/
	...
	this.count_key_instances = function(theObject,key, count) {
	    if(theObject instanceof Array) {
		for(var i = 0; i < theObject.length; i++) {
		    count = self.count_key_instances(theObject[i], key, count);
		}
	    }
	    else
	    {
		 for(var prop in theObject) {
		    if(theObject[prop] instanceof Array && prop === key){
			count += theObject[prop].length;
		    }
		    if(theObject[prop] instanceof Object || theObject[prop] instanceof Array){
			count = self.count_key_instances(theObject[prop], key, count);
		    }
		}
	    }
	    return count;
