	...
	this.render_shares = function(){
		...
		j(self.shares).each(function(i){
			if(j("share_type", this).text() == "3") //public
				j("ul.shares_list").append('<li>Public share link: <a href="'+file_share_url+'/s/'+j("token", this).text()+' "onclick="window.open(this.href,\'targetWindow\',\'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=400,height=400\'); return false;" >'+j("token", this).text()+'</a></li>');
