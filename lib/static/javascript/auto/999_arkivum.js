	...
	this.render_shares = function(){
		...
		j(self.shares).each(function(i){
			if(j("share_type", this).text() == "3") //public
				j("ul.shares_list").append('<li>Public share link: <a href="'+file_share_url+'/s/'+j("token", this).text()+'"
                onclick="window.open(this.href,'targetWindow',
                                   'toolbar=no,
                                    ...
                                    width=400,
                                    height=400');"
>'+j("token", this).text()+'</a></li>');
			if(j("share_type", this).text() == "0") //user
				j("ul.shares_list").append('<li>User share with <a href="'+file_share_url+'/apps/files/?dir=/'+eprintid+'"
 onclick="window.open(this.href,'targetWindow',
                                   'toolbar=no,
                                    location=no,
                                    status=no,
                                    ...
                                    width=400,
                                    height=400');"
>'+j("share_with_displayname", this).text()+'</a></li>');
...
//			data.password = j("input[name='share_pw']").val();
	...
	this.render_user = function(){
		j("p.oc_user").html( );
		j(self.shares).each(function(i){
			j("ul.shares_list").append('<li><a href="'+file_share_url+'/'+j("token", this).text()+'"
           onclick="window.open(this.href,'targetWindow',
                                   'toolbar=no,
                                    location=no,
                                    status=no,
